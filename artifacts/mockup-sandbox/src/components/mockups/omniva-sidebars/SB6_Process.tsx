...
const steps = [
  { n: '1', title: 'Strategy', body: 'We map your customers, goals, and competitive landscape before a single pixel is placed.' },
  { n: '2', title: 'Design', body: 'Wireframes, visual design, and UX flows — all reviewed and refined with your feedback.' },
  { n: '3', title: 'Build', body: 'Clean, fast, mobile-first development with no bloated templates.' },
  { n: '4', title: 'Launch', body: 'Go live with confidence. Hosting, DNS, and testing all handled.' },
  { n: '5', title: 'Grow', body: 'SEO, email, and AI tools that keep working after you launch.' },
];
...
export default function SB6_Process() {
  return (
    <div style={{ fontFamily: SF, margin: 0, padding: 0, overflowX: 'hidden', background: '#fff' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '72px 40px', display: 'flex', gap: 56, alignItems: 'flex-start' }}>
        ...
        <aside style={{ flex: '0 0 260px', position: 'sticky', top: 72 }}>
          <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#6e6e73', margin: '0 0 20px' }}>How it works</p>
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', left: 16, top: 24, bottom: 24, width: 2, background: 'linear-gradient(180deg, #c899e2, #f4a0b8, #ffb347)' }} />
            {steps.map(({ n, title, body }, i) => (
              <div key={n} style={{ display: 'flex', gap: 18, marginBottom: i < steps.length - 1 ? 24 : 0, position: 'relative' }}>
                ...
                <div style={{ paddingTop: 6 }}>
                  <p style={{ fontSize: 15, fontWeight: 700, color: '#1d1d1f', margin: '0 0 4px' }}>{title}</p>
                  <p style={{ fontSize: 13, color: '#6e6e73', lineHeight: 1.5, margin: 0 }}>{body}</p>
                </div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 28, background: '#f5f5f7', borderRadius: 14, padding: '16px', textAlign: 'center' }}>
            <button style={{ width: '100%', background: '#1d1d1f', color: '#fff', border: 'none', borderRadius: 980, padding: '10px 0', fontSize: 13, fontWeight: 600, fontFamily: SF, cursor: 'pointer' }}>
