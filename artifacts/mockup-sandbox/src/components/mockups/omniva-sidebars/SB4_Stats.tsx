...
const stats = [
  { num: '+15%', label: 'Lift in subscriptions', sub: 'Wells Fargo product redesign' },
  { num: '+22%', label: 'User engagement increase', sub: 'Deloitte UX overhaul' },
  { num: '90+', label: 'Apps using the design system', sub: 'Enterprise-scale adoption' },
  { num: '20+', label: 'Years of experience', sub: 'Design, UX, strategy' },
];
...
export default function SB4_Stats() {
  return (
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '72px 40px', display: 'flex', gap: 56, alignItems: 'flex-start' }}>
        ...
        <aside style={{ flex: '0 0 260px', position: 'sticky', top: 72 }}>
          ...
          {stats.map(({ num, label, sub }, i) => (
            <div key={num} style={{ borderTop: i === 0 ? 'none' : '1px solid #e5e5ea', paddingTop: i === 0 ? 0 : 20, marginBottom: 20 }}>
              <p style={{ fontSize: 44, fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.0, margin: '0 0 4px', background: 'linear-gradient(135deg, #c899e2, #f4a0b8, #ffb347)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                {num}
              </p>
          ...
          <div style={{ marginTop: 8, background: '#f5f5f7', borderRadius: 14, padding: '16px', textAlign: 'center' }}>
