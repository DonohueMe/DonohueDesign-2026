...
export default function OmnivaLayoutsCopyLlhliO4x() {
  return (
    <div style={{ fontFamily: SF, margin: 0, padding: 0, background: '#fff' }}>
      ...
      <section style={{ height: 600, background: HERO_BG, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '80px 40px 60px' }}>
        <h1 style={{ fontSize: 'clamp(48px, 7vw, 80px)', fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.5, color: '#1d1d1f', margin: '0 0 18px' }}>
          Websites that sell.<br />AI that closes.
        </h1>
      </section>
          ...
          {sections.map(({ h1 }, si) => (
            <React.Fragment key={si}>
              <div style={{ marginBottom: 56 }}>
                <div style={{ fontSize: 'clamp(27px, 3.5vw, 35px)', lineHeight: 1.32, margin: '0 0 40px' }}>
                  <h2 style={{ fontWeight: 800, letterSpacing: '-0.03em', color: '#1d1d1f', margin: 0, display: 'inline', fontSize: 'inherit', lineHeight: 'inherit' }}>
                    {h1}
                  </h2>
                  {' '}
                  <span style={{ fontWeight: 700, ...(GRAD_WARM as any) }}>
        ...
        <aside style={{ flex: '0 0 300px', position: 'sticky', top: 24, display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          {serviceCards.map(({ title, Icon, description, items }) => (
            <div key={title} style={{ background: '#fff', borderRadius: 18, padding: '20px 18px 52px', marginBottom: 10, border: '1px solid #e5e5ea', position: 'relative' }}>
              ...
              {items.map(item => (
                <p key={item} style={{ fontSize: 17, color: '#2997ff', margin: '0 0 3px', lineHeight: '27px' }}>{item}</p>
              ))}
            </div>
          ))}
