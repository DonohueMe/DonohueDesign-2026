          ...
          {sections.map(({ h1 }, si) => (
            <React.Fragment key={si}>
              <div style={{ marginBottom: 56 }}>
                <h1 style={{ fontSize: 'clamp(26px, 2.8vw, 36px)', fontWeight: 800, letterSpacing: '-0.04em', color: '#1d1d1f', margin: '0 0 40px', lineHeight: 1.3 }}>
                  {h1}<br />
        ...
        <aside style={{ flex: '0 0 300px', position: 'sticky', top: 24, display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          {serviceCards.map(({ title, Icon, description, items }) => (
            <div key={title} style={{ background: '#fff', borderRadius: 18, padding: '20px 18px 52px', marginBottom: 10, border: '1px solid #e5e5ea', position: 'relative' }}>
              ...
              {description && (
                <p style={{ fontSize: 17, color: '#6e6e73', margin: '0 0 10px', lineHeight: 1.55 }}>{description}</p>
              )}
              {items.map(item => (
                <p key={item} style={{ fontSize: 17, color: '#2997ff', margin: '0 0 3px', lineHeight: '22.3px' }}>{item}</p>
              ))}
            </div>
          ))}
