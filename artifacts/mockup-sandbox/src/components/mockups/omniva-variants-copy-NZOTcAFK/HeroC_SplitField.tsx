...
const GRAD_LEFT = [
  'radial-gradient(ellipse at 20% 30%, rgba(196,153,226,0.90) 0%, transparent 50%)',
  'radial-gradient(ellipse at 80% 70%, rgba(255,185,120,0.85) 0%, transparent 50%)',
  'radial-gradient(ellipse at 50% 55%, rgba(248,140,170,0.70) 0%, transparent 50%)',
  '#f5eef8',
].join(',');
...
export function HeroC_SplitField() {
  return (
    <div style={{ fontFamily: SF, margin: 0, padding: 0, overflowX: 'hidden' }}>
      <StickyNav />

      {/* Hero — two halves side by side */}
      <section style={{ display: 'flex', minHeight: 760 }}>

        {/* Left — pastel gradient, location + tagline */}
        <div style={{
          flex: 1,
          ...
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(29,29,31,0.50)', margin: '0 0 16px' }}>Santa Rosa, CA</p>
          <h1 style={{ fontSize: 'clamp(40px, 5.5vw, 68px)', fontWeight: 800, lineHeight: 1.0, letterSpacing: '-0.045em', color: '#1d1d1f', margin: '0 0 20px' }}>
            Website<br />Designer
          </h1>
          <p style={{ fontSize: 17, color: 'rgba(29,29,31,0.65)', lineHeight: 1.55, maxWidth: 340, margin: 0 }}>
            Strategy-driven web design and digital marketing for Sonoma County businesses.
          </p>
        </div>

        {/* Right — dark navy, credentials + CTA */}
        <div style={{
          flex: 1,
          background: '#0d1535',
          display: 'flex',
          ...
          <div style={{ display: 'flex', gap: 32, marginBottom: 32 }}>
            {[['20+', 'Years'], ['90+', 'Apps'], ['+22%', 'Engagement']].map(([n, l]) => (
              <div key={l}>
                <p style={{ fontSize: 22, fontWeight: 800, color: '#fff', margin: 0, letterSpacing: '-0.03em' }}>{n}</p>
                <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', margin: 0, fontWeight: 500 }}>{l}</p>
              </div>
            ))}
          </div>
          <button style={{ alignSelf: 'flex-start', background: '#fff', color: '#0d1535', border: 'none', borderRadius: 980, padding: '13px 26px', fontSize: 15, fontWeight: 700, fontFamily: SF, cursor: 'pointer' }}>
            Get started
          </button>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 40, alignItems: 'start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              {[
                ...
                { h: 'What I Do', paras: ["I offer website design, website development, and website redesign for local businesses \u2014 from tasting rooms along Highway 12 to shops in Railroad Square to service companies in Rincon Valley. Every site I build starts with research and ends with something that actually performs.", "I also handle e-commerce website design, SEO-optimized website design, UX/UI design and strategy, and website maintenance."] },
                { h: 'Beyond the Website', paras: ["Your site is the foundation. I also offer local SEO services, email marketing automation, and AI-powered tools \u2014 AI chat assistants, AI receptionists, and AI voice assistants \u2014 that capture leads and handle customer questions even when you\u2019re closed.", "On the brand side: logo design, brand identity design, brochure and flyer design, infographic design, and presentation design. Everything matches your website."] },
                { h: 'Serving Santa Rosa and Sonoma County', paras: ["I work with businesses across Santa Rosa and throughout Sonoma County, including Windsor, Rohnert Park, Petaluma, Healdsburg, and Sebastopol.", "If your website isn\u2019t bringing in the calls and customers it should, the issue is usually strategy. Let\u2019s figure out what\u2019s wrong and fix it. No jargon. No pressure."] },
              ].map(({ h, paras }, si) => (
                <div key={si}>
