    ...
    <div style={{ fontFamily: SF, margin: 0, padding: 0, background: '#fff' }}>
      ...
      <div style={{ display: 'flex', gap: 52, padding: '60px 40px 80px', maxWidth: 1100, margin: '0 auto', alignItems: 'flex-start' }}>
        ...
        <aside style={{ flex: '0 0 200px', position: 'sticky', top: 24, display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          {/* How it works — blue-to-teal gradient card */}
          <div style={{
            ...
            <p style={{ fontSize: 9.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.13em', color: 'rgba(255,255,255,0.60)', margin: '0 0 18px' }}>How it works</p>
            {timelineSteps.map(({ label, body }, i) => (
              <div key={label} style={{ display: 'flex', gap: 12, marginBottom: i < timelineSteps.length - 1 ? 18 : 0 }}>
                <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'rgba(255,255,255,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
                  <span style={{ fontSize: 11, fontWeight: 800, color: '#fff' }}>{i + 1}</span>
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#fff', margin: '0 0 3px', lineHeight: 1.2 }}>{label}</p>
                  <p style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.65)', margin: 0, lineHeight: 1.55 }}>{body}</p>
                </div>
              </div>
            ))}
            <button style={{ marginTop: 20, width: '100%', background: '#fff', color: '#1a6fd4', border: 'none', borderRadius: 980, padding: '13px 0', fontSize: 13, fontWeight: 700, fontFamily: SF, cursor: 'pointer' }}>
