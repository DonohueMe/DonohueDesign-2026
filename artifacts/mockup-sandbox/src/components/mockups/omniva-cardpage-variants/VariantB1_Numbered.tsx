...
const services = [
  { num: '01', tint: '#2997ff', kicker: 'Website Designer', title: 'Your website. Working harder than ever.', copy: "A well-designed website isn't just attractive — it's strategic. Every layout choice, call-to-action, and navigation path should guide visitors toward a clear goal. In 2025, that means mobile-first design, fast load times, and content built around what your customers actually need — not what looks impressive in a portfolio.", items: ['Website Design & Strategy', 'UX/UI Design', 'Website Development'], reverse: false },
  { num: '02', tint: '#34c759', kicker: 'Digital Marketing', title: 'Get found. Stay top of mind.', copy: 'Email campaigns and local SEO that put your business in front of the right people at the right time.', items: ['Email Marketing Automation', 'Local SEO Services (GMB)'], reverse: true },
  { num: '03', tint: '#c898e2', kicker: 'Internet Marketing Service', title: 'AI that answers, books, and converts — 24/7.', copy: 'Enterprise-grade automation tools built for small businesses — so you never miss a lead.', items: ['AI Receptionist', 'AI Voice Assistant', 'AI Chat Assistant'], reverse: false },
  { num: '04', tint: '#f09030', kicker: 'Graphic Designer', title: 'A brand as strong as your business.', copy: 'Everything matches your website — same colors, same feel, same level of quality across every touchpoint.', items: ['Email Design', 'Logo Design', 'Brochure & Flyer Design', 'Presentation Design'], reverse: true },
];

// VARIANT B1 — Numbered tiles + colored bars
export function VariantB1_Numbered() {
  return (
    <div style={{ fontFamily: SF, margin: 0, padding: 0, background: '#fff', minHeight: '100vh' }}>
      <section style={{
        ...
        {/* gradient bar across bottom of hero */}
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 6, background: 'linear-gradient(90deg, #2997ff, #34c759, #c898e2, #f09030)' }} />
      ...
      <div style={{ background: '#f5f5f7', padding: '60px 20px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 48 }}>
          {services.map(({ num, tint, kicker, title, copy, items, reverse }) => (
            <div key={num} style={{ background: '#fff', borderRadius: 20, padding: '52px 72px', position: 'relative', overflow: 'hidden', display: 'flex', gap: 80, alignItems: 'center', flexDirection: reverse ? 'row-reverse' : 'row' }}>
              {/* Color bar accent */}
              <div style={{ position: 'absolute', top: 0, bottom: 0, [reverse ? 'right' : 'left']: 0, width: 6, background: tint } as React.CSSProperties} />
              <div style={{ flex: '0 0 420px', position: 'relative' }}>
                {/* Giant number */}
                <p style={{ fontSize: 96, fontWeight: 800, letterSpacing: '-0.05em', color: tint, opacity: 0.18, margin: '0 0 -20px', lineHeight: 1, fontFamily: SF }}>{num}</p>
