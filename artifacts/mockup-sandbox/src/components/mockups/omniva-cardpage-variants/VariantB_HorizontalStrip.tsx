...
export function VariantK_HorizontalStrip() {
  return (
    <div style={{ fontFamily: SF, margin: 0, padding: 0, background: '#fff', minHeight: '100vh' }}>
      ...
      {/* Bio + service tiles with right-side fun sticker sidebar */}
      <section style={{ background: '#fff', padding: '40px 20px 60px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 280px', gap: 32, alignItems: 'start' }}>

          {/* Main column: bio + tiles */}
          <div>
            <div style={{ textAlign: 'center', padding: '0 60px 48px' }}>
              <h1 style={{ fontSize: 'clamp(36px, 4.5vw, 56px)', fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.05, color: '#1d1d1f', margin: '40px 0 24px' }}>
          ...
          {/* Right sidebar — fun sticker-style ad cards */}
          <aside style={{ position: 'sticky', top: 24, display: 'flex', flexDirection: 'column', gap: 14, marginTop: 40 }}>
            {[
              {
              ...
              <div key={eyebrow} style={{ background: bg, borderRadius: 20, padding: '18px 20px 16px', position: 'relative', overflow: 'hidden' }}>
                ...
                <h3 style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-0.025em', color: ink, margin: '0 0 14px', lineHeight: 1.05, whiteSpace: 'pre-line' }}>
                  {title}
                </h3>
