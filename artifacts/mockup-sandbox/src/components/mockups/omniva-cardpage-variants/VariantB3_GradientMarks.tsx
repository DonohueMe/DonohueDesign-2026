...
const ICONS: Record<string, React.ReactNode> = {
  browser: <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="16" rx="2"/><path d="M3 9h18"/></svg>,
  envelope: <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg>,
  spark: <svg width="34" height="34" viewBox="0 0 24 24" fill="white"><path d="M12 2l1.5 5.5L19 9l-5.5 1.5L12 16l-1.5-5.5L5 9l5.5-1.5z"/></svg>,
  palette: <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3a9 9 0 0 0 0 18c1.1 0 2-.9 2-2 0-.5-.2-1-.6-1.4-.4-.4-.6-.9-.6-1.4 0-1.1.9-2 2-2H17a4 4 0 0 0 4-4c0-4.4-4-8-9-8z"/></svg>,
};

const services = [
  { icon: 'browser', tint: '#2997ff', grad: 'linear-gradient(135deg, #2997ff, #5ac8fa)', kicker: 'Website Designer', title: 'Your website. Working harder than ever.', copy: "A well-designed website isn't just attractive — it's strategic. Every layout choice, call-to-action, and navigation path should guide visitors toward a clear goal. In 2025, that means mobile-first design, fast load times, and content built around what your customers actually need — not what looks impressive in a portfolio.", items: ['Website Design & Strategy', 'UX/UI Design', 'Website Development'], reverse: false },
  { icon: 'envelope', tint: '#34c759', grad: 'linear-gradient(135deg, #34c759, #28c9a8)', kicker: 'Digital Marketing', title: 'Get found. Stay top of mind.', copy: 'Email campaigns and local SEO that put your business in front of the right people at the right time.', items: ['Email Marketing Automation', 'Local SEO Services (GMB)'], reverse: true },
  { icon: 'spark', tint: '#c898e2', grad: 'linear-gradient(135deg, #c898e2, #f4a0b8)', kicker: 'Internet Marketing Service', title: 'AI that answers, books, and converts — 24/7.', copy: 'Enterprise-grade automation tools built for small businesses — so you never miss a lead.', items: ['AI Receptionist', 'AI Voice Assistant', 'AI Chat Assistant'], reverse: false },
  { icon: 'palette', tint: '#f09030', grad: 'linear-gradient(135deg, #f09030, #f4a0b8)', kicker: 'Graphic Designer', title: 'A brand as strong as your business.', copy: 'Everything matches your website — same colors, same feel, same level of quality across every touchpoint.', items: ['Email Design', 'Logo Design', 'Brochure & Flyer Design', 'Presentation Design'], reverse: true },
];

// VARIANT B3 — Gradient mark tiles with radial glow
export function VariantB3_GradientMarks() {
  return (
    <div style={{ fontFamily: SF, margin: 0, padding: 0, background: '#fff', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#f5f5f7', padding: '60px 20px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 48 }}>
          {services.map(({ icon, tint, grad, kicker, title, copy, items, reverse }) => (
            <div key={kicker} style={{ background: '#fff', borderRadius: 20, padding: '52px 72px', display: 'flex', gap: 80, alignItems: 'center', flexDirection: reverse ? 'row-reverse' : 'row', position: 'relative', overflow: 'hidden' }}>
              {/* radial glow background tied to brand color */}
              <div style={{ position: 'absolute', top: -120, [reverse ? 'left' : 'right']: -120, width: 380, height: 380, borderRadius: '50%', background: `radial-gradient(circle, ${tint}33 0%, transparent 65%)`, pointerEvents: 'none' } as React.CSSProperties} />
              <div style={{ flex: '0 0 420px', position: 'relative' }}>
                {/* Gradient mark tile */}
                <div style={{ width: 88, height: 88, borderRadius: 22, background: grad, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 24, boxShadow: `0 12px 36px ${tint}55` }}>{ICONS[icon]}</div>
              ...
              <div style={{ flex: 1, [reverse ? 'borderRight' : 'borderLeft']: '1px solid #e5e5ea', [reverse ? 'paddingRight' : 'paddingLeft']: 60, position: 'relative' } as React.CSSProperties}>
                {items.map(item => (
                  <div key={item} style={{ borderBottom: '1px solid #e5e5ea', padding: '16px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 16, color: '#1d1d1f' }}>{item}</span>
                    <span style={{ width: 28, height: 28, borderRadius: '50%', background: grad, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 14 }}>›</span>
                  </div>
                ))}
              </div>
            </div>
